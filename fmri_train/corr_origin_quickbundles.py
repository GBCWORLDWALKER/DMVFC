...
def get_corr(h5_directory,device):
    ...
    h5_files = glob.glob(os.path.join(h5_directory, "*_fmri_clusters_endpoint_quickbundles.h5"))
    ...
    for h5_file_path in h5_files:
        ...
        with h5py.File(h5_file_path, 'r') as f:
            ...
            for i in range(num_clusters):
                ...
                upper_triangle = torch.triu(cor_matrix, diagonal=1)
                upper_triangle_1 = torch.triu(cor_matrix_1, diagonal=1)
                
                # Calculate the average of the upper triangular part
                cor_average = torch.mean(upper_triangle[upper_triangle != 0])
                cor_average_1 = torch.mean(upper_triangle_1[upper_triangle_1 != 0])
                cor_average=torch.max(cor_average,cor_average_1)
    ...
    results_file = os.path.join(h5_directory, "correlation_results_endpoint_quickbundles.txt")
