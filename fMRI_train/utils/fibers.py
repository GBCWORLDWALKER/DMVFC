...
class FiberPair(data.Dataset):
    ...
    def __init__(self, vec, subID, index_pair,subID_counts,fmri_similarity_path,dmri_similarity_path,bundle,transform=None, embedding_surf=False,funct=True,window=128,model_b=None):
    ...
    def get_epoch_similarity(self, epoch):
        ...
        if self.fmri_similarity_path !="None" and self.fmri_similarity_path is not None:
            ...
            self.similarity=self.fmri_similarity.to(torch.float)
        
            
        elif self.dmri_similarity_path is not None:
            self.dmri_similarity=torch.load(os.path.join(self.dmri_similarity_path,f"dmri_{self.bundle}_epoch_{epoch}.pt"),weights_only=False,map_location='cuda')
            self.similarity=self.dmri_similarity.to(torch.float)
