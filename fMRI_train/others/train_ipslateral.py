    ...
    if args.mode == 'train_full':
        ...
        if args.ro:
            ...
            if id_reject is not None:
                ...
                preds_fs_surf_rej = preds_fs_surf[~mask]
                probs_fs_surf_rej = probs_fs_surf[~mask]
                ...
                maskc[id_reject] = 0
                pd_c = wma.filter.mask(input_pds[0], maskc, preserve_point_data=True, preserve_cell_data=False,
                                       verbose=False)

                output_wm = os.path.join(outdir, 'all', 'all.vtp')
                if not os.path.exists(os.path.join(outdir, 'all')):
                    os.makedirs(os.path.join(outdir, 'all'))
                wma.io.write_polydata(pd_c, output_wm)
