...
def train_model(model,dataset , criteria, optimizers, schedulers, params, fiber_array):
    ...
    log = params['log']
    pretrained_net = params['model_files'][0]
    ...
    if pretrain:
        ...
        pretrained_model = pretraining(model, copy.deepcopy(dataset), criteria[0], optimizers[1], schedulers[1], pretrain_epochs, params)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_num, data in enumerate(dataloader):
            ...
            with torch.set_grad_enabled(True):
                outputs, clusters, _, _, _, dis_point = model(input1, input2, freeze=params['freeze'])
                if surf_flag:
...
def pretraining(model, dataset, criterion, optimizer, scheduler, num_epochs, params):
    # Note the time
    since = time.time()
...
def calculate_predictions_roi(model, dataset, params, surf_cluster=None):
    ...
    subnum = None
    model.eval()
    if params['similarity_path'] is not None:
        dataset.get_epoch_similarity(0)
        dataset.get_epoch_similarity(params['epochs_pretrain'])
    else:
        dataset.get_epoch_similarity(0)   
