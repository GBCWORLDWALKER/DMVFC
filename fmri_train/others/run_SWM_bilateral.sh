...
do
   ...
   if [ ! -f $inputfolder/$runstr/iter$i/cluster_00001.vtp ]; then
      $2 python train_bilateral.py --pretrain True --fs True --surf True --ro True -indir $input -outdir $inputfolder/$runstr/iter$i/ -p $p --num_clusters $num_clusters --epochs_pretrain ${ep} --epochs 1
   fi
