...
def process_args_test(args):
    ...
    if args.outputDirectory is None:
        raise ValueError("请提供有效的 --outputDirectory 参数")
    if args.output_name is None:
        raise ValueError("请提供有效的 --output_name 参数")
    params = {}
    params['old_endpoint'] = args.old_endpoint
    params['figure'] = args.figure
    params['test'] = args.test
    params['mapping'] = args.mapping
    params['none_mapping'] = args.none_mapping
    params['presaved_path'] = args.presaved_path
    params['num_clusters'] = args.num_clusters
    params['not_pre_saved_results'] = args.not_pre_saved_results
    params['data_parallel'] = args.data_parallel
    if args.FA_base_path is not None:
        params['FA_base_path'] = args.FA_base_path

    params['model_b'] = args.model_b
    ...
    params['outputDirectory'] = os.path.join(args.outputDirectory,args.output_name)
    os.makedirs(params['outputDirectory'], exist_ok=True)
    report_file = os.path.join(params['outputDirectory'], "log_fmri.txt")
    ...
    params['multi_view_test'] = args.multi_view_test
    params['trained_net_b'] = args.trained_net_b
    params['origin'] = args.origin
    params['trained_net_mapping'] = args.trained_net_mapping
    return params

def process_args(args):
    params = {}

    params['demo'] = args.demo
    params['mapping'] = args.mapping
    params['data_parallel'] = args.data_parallel
    params['index_pair_path'] = args.index_pair_path
    params['lambda_a'] = args.lambda_a
    params['lambda_b'] = args.lambda_b
    ...
    params['single_prepared'] = args.single_prepared
    ...
    params['tol'] = args.tol
    params['strategy_initial'] = args.strategy_initial
    params['loss_strategy'] = args.loss_strategy
    # Number of epochs
    ...
    params['epochs_mapping'] = args.epochs_mapping
    # Adam params
    # Weight decay
    ...
    if args.pretrain:
        trained_pretrain_net_path_a = "None"
        trained_pretrain_net_path_b = "None"
    else:
        trained_pretrain_net_path_a = args.pretrained_net_a
        trained_pretrain_net_path_b = args.pretrained_net_b
        if not os.path.isfile(trained_pretrain_net_path_a):
            print("Error: No pretrained weights at %s" % trained_pretrain_net_path_a)
            exit()
        if not os.path.isfile(trained_pretrain_net_path_b):
            print("Error: No pretrained weights at %s" % trained_pretrain_net_path_b)
            exit()
    ...
    if wandb_group is not None:
        ...
        wandb.config.update({
        "pretrain_net_path_a": os.path.join(args.outputDirectory, 'models', f'{run_name}_pretrain_a_k{args.num_clusters}.pt'),
        "final_net_path_a": os.path.join(args.outputDirectory, 'models', f'{run_name}_final_a_k{args.num_clusters}.pt'),
        "pretrain_net_path_b": os.path.join(args.outputDirectory, 'models', f'{run_name}_pretrain_b_k{args.num_clusters}.pt'),
        "final_net_path_b": os.path.join(args.outputDirectory, 'models', f'{run_name}_final_b_k{args.num_clusters}.pt')
    })

    # Assign the paths to variables for use in the current run
        pretrain_net_path_a = wandb.config.pretrain_net_path_a
        final_net_path_a = wandb.config.final_net_path_a
        pretrain_net_path_b = wandb.config.pretrain_net_path_b
        final_net_path_b = wandb.config.final_net_path_b
    else:
        pretrain_net_path_a=os.path.join(args.outputDirectory, 'models', f'{run_name}_pretrain_a_k{args.num_clusters}.pt')
        final_net_path_a=os.path.join(args.outputDirectory, 'models', f'{run_name}_final_a_k{args.num_clusters}.pt')
        pretrain_net_path_b=os.path.join(args.outputDirectory, 'models', f'{run_name}_pretrain_b_k{args.num_clusters}.pt')
        final_net_path_b=os.path.join(args.outputDirectory, 'models', f'{run_name}_final_b_k{args.num_clusters}.pt')
        mapping_net_path=os.path.join(args.outputDirectory, 'models', f'{run_name}_mapping_k{args.num_clusters}.pt')


    model_files = [trained_pretrain_net_path_a,trained_pretrain_net_path_b, pretrain_net_path_a, pretrain_net_path_b, final_net_path_a, final_net_path_b,mapping_net_path]
