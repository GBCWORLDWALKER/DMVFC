...
class FiberPair(data.Dataset):
    ...
    def __init__(self, vec, subID, index_pair,subID_counts,fmri_similarity_path,dmri_similarity_path,bundle,transform=None, embedding_surf=False,window=128,fmri=False,model_b=None):
        ...
        self.bundle=bundle
        self.fmri_similarity_path=fmri_similarity_path
    ...
    def get_epoch_similarity(self, epoch):
        ...
        """
        #TODO: check this when you change dataset
        self.epoch_index2=self.index_pair[epoch][:len(self.vec)]
        if self.fmri_similarity_path !="None" and self.fmri_similarity_path is not None:
            self.fmri_similarity = torch.load(os.path.join(self.fmri_similarity_path, f"coor_fiber_{epoch}.pt"),weights_only=False,map_location='cuda')
            self.fmri_similarity = 1-self.fmri_similarity
            self.similarity = self.fmri_similarity.to(dtype=torch.float)
            
        elif self.dmri_similarity_path is not None and self.dmri_similarity_path != "None":
            self.similarity=torch.load(os.path.join(self.dmri_similarity_path,f"dmri_{self.bundle}_epoch_{epoch}.pt"),weights_only=False,map_location='cuda')
            self.similarity=self.similarity.to(dtype=torch.float)
    ...
    def __getitem__(self, index: int):
        ...
        if self.fmri and self.model_b=="st":
            r=torch.randint(0,self.vec.shape[2]-self.window,(1,))
            fiber1 = torch.tensor(fiber1.T, dtype=torch.float).unsqueeze(0).unsqueeze(-1)[:,r:r+self.window,:,:]
            fiber2 = torch.tensor(fiber2.T, dtype=torch.float).unsqueeze(0).unsqueeze(-1)[:,r:r+self.window,:,:]
        else:
