...
def main_train(params,args,writer):
    ...
    if params['demo']:
        fmri_path_list = fmri_path_list[:1]
    fmri_array=[]
    ...
    if params['model_b']=="dgcnn":
        fmri_array=fmri_array[:,[0,-1],::2].reshape(-1,40,30)
        
        
        
    dataset_a = fibers.FiberPair(
        fiber_array.fiber_array_ras,
        ...
        dmri_similarity_path=params['dmri_similarity_path'],
                               ...
                               bundle=params['bundle'],fmri=False)#(num,15,3)
    dataset_b=fibers.FiberPair(
        ...
        dmri_similarity_path=None,
                               transform=transforms.Compose([transforms.ToTensor()]),
                               embedding_surf=params['embedding_surf'],
                               bundle=params['bundle'],fmri=True,model_b=params['model_b'])#(num,15,3)
    params['dataset_size'] = len(dataset_a)

    dataloader_a = torch.utils.data.DataLoader(dataset_a, batch_size=params["batch_size"], shuffle=False, num_workers=10)
    dataloader_b = torch.utils.data.DataLoader(dataset_b, batch_size=params["batch_size"], shuffle=False, num_workers=10)
    dataloader_a_entire = torch.utils.data.DataLoader(dataset_a, batch_size=int(params["batch_size"]/2), shuffle=False, num_workers=10)
    dataloader_b_entire = torch.utils.data.DataLoader(dataset_b, batch_size=int(params["batch_size"]/2), shuffle=False, num_workers=10)

    device = torch.device(f"cuda:{params['GPU'][0]}" if torch.cuda.is_available() else "cpu")
...
#define mdoel a
    ...
    model_a = nets.DGCNN(k=args.k, input_channel=3, num_clusters=params["num_clusters"], embedding_dimension=params["embedding_dimension"], idx=idx,device=device)

    num_pa = nets.count_parameters(model_a)
    utils.print_both(params['log'], "Number of a network parameters:\t" + str(num_pa))

    optimizer_a   = optim.Adam(filter(lambda p: p.requires_grad, model_a.parameters()), lr=params["rate"], weight_decay=params["weight"])
    optimizer_pretrain_a = optim.Adam(filter(lambda p: p.requires_grad, model_a.parameters()), lr=params["rate_pretrain"], weight_decay=params["weight_pretrain"])
    optimizers_a = [optimizer_a, optimizer_pretrain_a]

#define model b
    if params['model_b'] == "st_gcn":
        model_b = st_gcn_fiber(1, 1, embedding_dimension=params["embedding_dimension"], num_clusters=params["num_clusters"])
    elif params['model_b'] == "dgcnn":
        idx_b = nets.get_dgcnn_idx(10, 40, params["batch_size"]).to(device) if args.idx else None
        model_b = nets.DGCNN( k=10, input_channel=30, num_clusters=params["num_clusters"], embedding_dimension=10, idx=idx_b,device=device)
    else:
        raise ValueError(f"Invalid model_b value: {params['model_b']}")
    
    
    
    if torch.cuda.device_count() > 1:
        device_ids = params['GPU']
        model_b = nn.DataParallel(model_b, device_ids=device_ids)
        model_a = nn.DataParallel(model_a, device_ids=device_ids)
    ...
    scheduler_a = lr_scheduler.StepLR(optimizer_a, step_size=params["sched_step"], gamma=params["sched_gamma"])
    scheduler_pretrain_a = lr_scheduler.StepLR(optimizer_pretrain_a, step_size=params["sched_step_pretrain"], gamma=params["sched_gamma_pretrain"])
    schedulers_a = [scheduler_a, scheduler_pretrain_a]
    ...
    # Train model
    model, model_b,preds_final, probs_final, surf_cluster = \
        training_functions_fiber_pair.train_model(model_a, model_b, dataset_a,dataset_b,criteria, optimizers_a, optimizers_b, schedulers_a, schedulers_b, params,fiber_array, writer)

    final_net_save = params['model_files'][4]
...
def main():    # Translate string entries to bool for parserffree]]
    ...
    parser.add_argument('-outdir',action="store", dest="outputDirectory",default="/data06/jinwang/journal_2024/output", help='Output folder of clustering results.')
    ...
    parser.add_argument('--GPU', default=[0], type=list, help='GPU number list')
    ...
    # parser.add_argument('--demo', default=False, type=str2bool, help='Demo mode')
    # parser.add_argument('--mapping', default=None, type=str, help='Path to the mapping file')
    # parser.add_argument('--data_parallel', default=False, type=str2bool, help='Use data parallelism')
    # parser.add_argument('--index_pair_path', default=None, type=str, help='Path to the index pair file')
    # parser.add_argument('--lambda_a', default=1.0, type=float, help='Weight for loss component A')
    # parser.add_argument('--lambda_b', default=1.0, type=float, help='Weight for loss component B')
    # parser.add_argument('--epochs_mapping', default=100, type=int, help='Number of epochs for mapping')
    # parser.add_argument('--model_b', default='st_gcn', choices=['st_gcn', 'dgcnn'], help='Model type for network B')
    args = parser.parse_args()
    
    # Load and merge YAML configuration
    args = load_config(args)
    if args.demo:
        args.output_name = args.output_name + "_demo"
