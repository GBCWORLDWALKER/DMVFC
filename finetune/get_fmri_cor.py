...
if __name__ == "__main__":
    ...
    path_list_endpoint = os.path.join(os.path.dirname(params['multi_view_test']),"endpoint_test")
    fmri_path_list_endpoint = [os.path.join(path_list_endpoint, subject,fmri_path) 
                    for subject in sorted(os.listdir(path_list_endpoint), key=numsort) for fmri_path in sorted(os.listdir(os.path.join(path_list_endpoint,subject))) 
    ...
    if params['origin'] is True:
        fmri_array_1=[]
        for fmri_path in fmri_path_list_endpoint:
            mat=sio.loadmat(fmri_path)
            fmri_array_1.append(mat['W_all'].transpose(2,1,0)[:,::2,:])
        fmri_array_1=np.concatenate(fmri_array_1,axis=0).reshape(-1,40,30)
    else:
    ...
    get_corr_origin(params['outputDirectory'])
