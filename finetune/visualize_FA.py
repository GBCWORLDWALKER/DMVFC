...
from utils import utils, nets_FA
...
def visualize_clusters(model, model_b, model_mapping, dataset, dataset_b, params):
    ...
    if params['none_mapping']:
        ...
        embeddings_tensor = torch.tensor(embeddings).to(params['device'])
        embeddings_b_tensor = torch.tensor(embeddings_b).to(params['device'])
        
        if params.get('FA_base_path') is not None:
            from utils.clustering import clustering_function
            
            clusters, x_dis, _ = clustering_function(
                embeddings_tensor, 
                weights=model.module.clustering.weight, 
                alpha=model.module.clustering.alpha,
                FA_base_path=params['FA_base_path'],
                fiber_array=fiber_array
            )
            
            clusters_b, x_dis_b, _ = clustering_function(
                embeddings_b_tensor,
                weights=model_b.module.clustering.weight,
                alpha=model_b.module.clustering.alpha,
                FA_base_path=params['FA_base_path'],
                fiber_array=fiber_array
            )
            ...
            clusters,_=model.module.clustering(embeddings_tensor)
            clusters_b,_=model_b.module.clustering(embeddings_b_tensor)
        
        embeddings_b=embeddings_b_tensor.cpu().detach().numpy()
        embeddings=embeddings_tensor.cpu().detach().numpy()
...
if __name__ == "__main__":
    ...
    parser.add_argument('-outputDirectory',action="store", dest="outputDirectory", default=None, help='output directory')
    ...
    parser.add_argument('--data_parallel', default=True, type=str2bool, help='data parallel or not')
    parser.add_argument('--model_b', default="dgcnn", type=str, help='model b')
    ...
    parser.add_argument('--FA_base_path', default=None, type=str, help='FA文件的基础路径，例如"/data06/jinwang/isbi/data/FA"')
    ...
    for subID in unique_subIDs:
        ...
        index_pair[subID] = generate_unique_pairs(vec, 2,device=params['device'], seed=1)
    ...
    if params['model_b']=="st":
        ...
        idx_b = nets_FA.get_dgcnn_idx(10, 40, params["batch_size"]).to(device)
        model_b = nets_FA.DGCNN(k=10, input_channel=30, num_clusters=num_clusters, embedding_dimension=embedding_dimension, idx=idx_b,device=device)
    ...
    idx = nets_FA.get_dgcnn_idx(5, params["num_points"], params["batch_size"]).to(device)
    model = nets_FA.DGCNN(k=5, input_channel=3, num_clusters=num_clusters, embedding_dimension=embedding_dimension, idx=idx,device=device)
    ...
    params['fiber_array'] = fiber_array
