...
def distance_prepare_single_tract(index_pair,subID_counts, fiber_array_ras, embedding_surf,epoch,filepath,bundle):
    ...
    epoch_device_pairs = [(ep,ep%devices) for ep in range(0,num_epochs)]
    num_fibers=len(fiber_array_ras)
    def process_epoch(epoch,deviceid):
        if os.path.exists(os.path.join(filepath,f'dmri_{bundle}_epoch_{epoch}.pt')):
            return
        ...
        similarities_tensor = torch.tensor(similarities, device=f'cuda:{deviceid}').to(torch.float32)
        torch.save(similarities_tensor,os.path.join(filepath,f'dmri_{bundle}_epoch_{epoch}.pt'))
        torch.cuda.empty_cache()
    Parallel(n_jobs=8)(delayed(process_epoch)(epoch,0) for epoch,deviceid in epoch_device_pairs)
...
def distance_prepare_tract(index_pair,subID_counts, fiber_array_ras,fmri_path_list,epoch,filepath):
    ...
    num_fibers=len(fiber_array_ras)
    devices=torch.cuda.device_count()
    epoch_device_pairs = [(ep,ep%devices) for ep in range(num_epochs)]
    def process_epoch(epoch,deviceid):
        similarities = np.zeros(num_fibers)
        j=0
        sump=0
    ...
    Parallel(n_jobs=devices)(delayed(process_epoch)(epoch,deviceid) for epoch,deviceid in epoch_device_pairs)
    print("Distance preparation completed for all epochs.")
# def distance_prepare(filepath,index_pair,subID_counts, fiber_array_ras, embedding_surf,fmri_path_list,epoch,params):
#     from joblib import Parallel, delayed
#     import numpy as np
...
def distance_prepare(filepath,file_path_final, index_pair, subID_counts, fiber_array_ras, embedding_surf, fmri_path_list, epoch, params):
    from joblib import Parallel, delayed
    import numpy as np
    num_epochs = epoch + 50
    ...
    os.makedirs(os.path.join(filepath,file_path_final),exist_ok=True)
    ...
    def process_epoch(epoch, device_id):
        ...
        # Load fmri_data directly to the specified device
        fmri_data = torch.tensor(
            sio.loadmat(fmri_path_list[epoch])['distances'],
            device=device,
            dtype=torch.float32
        )
        fmri_data=fmri_data.flatten()
        single_prepared_path = os.path.join(filepath, f'single_tract_epoch_{epoch}.pt')
        ...
        similarities_tensor = similarities.to(torch.float32)
        save_path = os.path.join(filepath,file_path_final, f"similarities_epoch_{epoch}.pt")
