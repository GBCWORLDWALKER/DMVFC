...
def visualize_test(params,fiber_array):
    ...
    for subID in unique_subIDs:
        ...
        index_pair[subID] = generate_unique_pairs(vec, 2,params['device'],seed=1)
    ...
    for fmri_path in fmri_path_list:
        ...
        fmri_array.append(f['W_all'].transpose(2,1,0))
    # for fmri_path in fmri_path_list:
    ...
    if params['old_endpoint']:
        ...
        for fmri_path in fmri_path_list_endpoint:
            mat=sio.loadmat(fmri_path)
            fmri_array_endpoint.append(mat['W_all'])
    ...
    for i in range(len(preds_final_list)):
        ...
        streamlines_1=fiber_array.fiber_array_ras
        streamlines_1=np.delete(streamlines_1, rejected_fibers, axis=0)
        ...
        print(f"Requested {num_clusters} clusters, but QuickBundles found {actual_num_clusters}")
        ...
        for subject_id in np.unique(dmri_array_id):
            ...
            subject_cluster_ours_arrays[subject_id]={}
            ...
            for cluster in range(actual_num_clusters):
                ...
                subject_cluster_ours_arrays[subject_id][cluster] = streamlines_1[cluster_mask]
        if i==0:
            ...
            for subject in range(len(subject_cluster_quickbundles_arrays)):
                ...
                for cluster in range(len(subject_cluster_quickbundles_arrays[subject])):
                    ...
                    output_vtk_path_ours = os.path.join(params['outputDirectory'], str(subject), f"{params['bundle']}_subject_{subject}_cluster_{cluster}_ours.vtk")
                    ...
                    if len(subject_cluster_ours_arrays[subject][cluster]) != 0:
                        save_vtk_streamlines(subject_cluster_ours_arrays[subject][cluster], output_vtk_path_ours)
