...
class ClusterlingLayer(nn.Module):
    ...
    def forward(self, x):
        # Normalize x and weight using L2 norm
        x_normalized = torch.mul(F.normalize(x, p=2, dim=1),100)
        weight_normalized = torch.mul(F.normalize(self.weight, p=2, dim=1),100)
        x = x_normalized.unsqueeze(1) - weight_normalized
        x = torch.mul(x, x)
        x = torch.sum(x, dim=2)
        x_dis = x
        x = 1.0 + (x / self.alpha)
...
class DGCNN(nn.Module):
    # from DGCNN's repo
    def __init__(self, k=5, input_channel=3, num_clusters=800, features_len=1024, embedding_dimension=10, bias=True, idx=None,device='cuda:0'):
    ...
    def forward(self, x1, x2, freeze=False):
        ...
        def forward_single(self, x):
            ...
            with torch.set_grad_enabled(not freeze):
                ...
                batch_size = x.size(0)
                x = get_graph_feature(self,x, k=self.k, idx=self.idx)
                x = self.conv1(x)
                x1 = x.max(dim=-1, keepdim=False)[0]

                x = get_graph_feature(self,x1, k=self.k, idx=self.idx)
                x = self.conv2(x)
                x2 = x.max(dim=-1, keepdim=False)[0]

                x = get_graph_feature(self,x2, k=self.k, idx=self.idx)
                x = self.conv3(x)
                x3 = x.max(dim=-1, keepdim=False)[0]

                x = get_graph_feature(self,x3, k=self.k, idx=self.idx)
                x = self.conv4(x)
...
def get_dgcnn_idx(k, num_points, batch):

    idxf = torch.zeros((num_points, k), dtype=torch.int64)
    if k == 5:
...
# Define MultiViewDGCNN class
# Define the parallel model using ModuleList
